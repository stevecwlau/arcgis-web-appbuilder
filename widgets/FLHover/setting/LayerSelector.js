...
define([
  ...
  return declare([_WidgetBase, _TemplatedMixin], {
    ...
    addLayerNode: function(layerInfo, level, toTableNode) {
      ...
      if(this.config.layerId === layerInfo.id) {
        checkedValue = true;
      }
      ckSelect = new CheckBox({
        checked: checkedValue,
        disabled: layerInfo.layerObject.declaredClass !== 'esri.layers.FeatureLayer',
    ...
    getLayerOptions: function() {
      ...
      for(var child in this._displayStateStorage) {
        if(this._displayStateStorage.hasOwnProperty(child) &&
          ...
          if(this._displayStateStorage[child].displayCK.checked){
            layerOptions = child;
          }
        }
      }
